...
class csvToEventsMatrix:
    def __init__(self, match_code="sassuolo_inter_1", pass_type="all"):
        ...
        with open("../Video Labeling/Data/events_Italy.json", encoding='utf-8') as file_events:
            data_events = json.load(file_events)
        with open("../Video Labeling/Data/chievo-juve-2018-08-18.json", encoding='utf-8') as file_events:
        ...
        events_pass_match = []
        with open("../Video Labeling/Data/" + match_code + ".csv") as file:
            for line in file.readlines():
                events_pass_match.append(line[:len(line)-1].split(";"))
        #Get the number of the frames in the video_match
        video = cv2.VideoCapture("../Video Labeling/static/" + match_code + ".mp4")
    ...
    def saveTargetMatrix(self, path="../../outputs/", drive=True):
        if self.pass_type != "all":
        ...
        if drive:
            ...
            file1 = drive.CreateFile({"title" : filename, "parents": [{"kind": "drive#fileLink", "id": '1VgGbMkJIiOYZgbG5CfAzDoi9KacHk_q1'}]})
